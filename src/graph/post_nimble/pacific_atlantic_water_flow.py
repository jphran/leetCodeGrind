...
class Solution:
    ...
    def pacificAtlantic(self, heights: list[list[int]]) -> list[list[int]]:
        ...
        pacific_drainage = self._all_connected_nodes(
            starts=pacific_ocean, heights=heights
        )
        ...
        atlantic_drainage = self._all_connected_nodes(
            starts=atlantic_ocean, heights=heights
        )
    ...
    def _all_connected_nodes(
        self, starts: list[list[int]], heights: list[list[int]]
    ) -> list[list[int]]:
        ...
        while len(to_visit.queue) != 0:
            ...
            for dir in self._nesw:
                ...
                if new_node in visited:
                    continue
                if (
                    len(heights) <= new_node[0]
                    or new_node[0] < 0
                    or len(heights) <= new_node[1]
                    or new_node[1] < 0
                ):
                    continue
